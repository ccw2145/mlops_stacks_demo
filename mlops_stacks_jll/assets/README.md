...
The databricks CLI bundles top level is defined by file `mlops_stacks_jll/databricks.yml`.
...
ML Resource Configurations in this directory:
 - model workflow (`mlops_stacks_jll/assets/model-workflow-asset.yml`)
 - batch inference workflow (`mlops_stacks_jll/assets/batch-inference-workflow-asset.yml`)
 - monitoring workflow (`mlops_stacks_jll/assets/monitoring-workflow-asset.yml`)
 - feature engineering workflow (`mlops_stacks_jll/assets/feature-engineering-workflow-asset.yml`)
 - model definition and experiment definition (`mlops_stacks_jll/assets/ml-artifacts-asset.yml`)
...
If you want to use different catalog names, please update the `targets` declared in the `mlops_stacks_jll/databricks.yml` and `mlops_stacks_jll/assets/ml-artifacts-asset.yml` files.
...
|-------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|----------------------|-------------------------------------|------------------------------------------------|
| dev         | The `dev` deployment target is used by ML engineers to deploy ML assets to development workspace with `dev` configs. The config is for ML project development purposes.                                                           | dev workspace        | dev-mlops_stacks_jll-model     | /dev-mlops_stacks_jll-experiment     |
| staging     | The `staging` deployment target is part of the CD pipeline. Latest main content will be deployed to staging workspace with `staging` config.                                                             | staging workspace    | staging-mlops_stacks_jll-model | /staging-mlops_stacks_jll-experiment |
| prod        | The `prod` deployment target is part of the CD pipeline. Latest release content will be deployed to prod workspace with `prod` config.                                                                      | prod workspace       | prod-mlops_stacks_jll-model    | /prod-mlops_stacks_jll-experiment    |
| test        | The `test` deployment target is part of the CI pipeline. For changes targeting the main branch, upon making a PR, an integration test will be triggered and ML assets deployed to the staging workspace defined under `test` deployment target. | staging workspace    | test-mlops_stacks_jll-model    | /test-mlops_stacks_jll-experiment    |
...
2. Complete the `TODO` in `mlops_stacks_jll/databricks.yml` to add the dev workspace URI under `targets.dev.workspace.host`.
...
Following this, update the batch_inference_job base parameters in `mlops_stacks_jll/assets/batch-inference-workflow-asset.yml` to pass
...
To get started, open `mlops_stacks_jll/assets/batch-inference-workflow-asset.yml`.  The file contains the ML asset definition of a batch inference job, like:
...
resources:
  jobs:
    batch_inference_job:
      name: ${bundle.target}-mlops_stacks_jll-batch-inference-job
      tasks:
...
The example above defines a Databricks job with name `${bundle.target}-mlops_stacks_jll-batch-inference-job`
that runs the notebook under `mlops_stacks_jll/deployment/batch_inference/notebooks/BatchInference.py` to regularly apply your ML model for batch inference. 
...
We specify a `batch_inference_job` under `assets/jobs` to define a databricks workflow with internal key `batch_inference_job` and job name `{bundle.target}-mlops_stacks_jll-batch-inference-job`.
...
`test-mlops_stacks_jll-batch-inference-job`. During the deployment of the `staging` environment config, the job name will be
`staging-mlops_stacks_jll-batch-inference-job`.
